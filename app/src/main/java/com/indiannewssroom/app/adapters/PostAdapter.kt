...
class PostAdapter (val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    class BigViewHolder(private val binding: BigHomeRowLayoutBinding) : RecyclerView.ViewHolder(binding.root) {

        fun bind(postData: PostData?, context: Context) {
            if (postData != null) {
                binding.txtposttitle.text = postData.title?.rendered
                binding.txtHomeCat.text = postData.embedded?.wpTerm?.get(0)?.get(0)?.name
                binding.txtPostTime.text = postData.date?.toDate()?.formatTo("dd MMM yyyy")
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        val bigView : BigHomeRowLayoutBinding

        return if (viewType == FIRST_VIEW_TYPE){
            bigView = BigHomeRowLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
