...
import com.indiannewssroom.app.databinding.HomeBigRowLayoutBinding
...
class HomeVerticalAdapter (val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    class BigViewHolder(private val binding: HomeBigRowLayoutBinding) : RecyclerView.ViewHolder(binding.root) {
...
//                postData.embedded.wpTerm[0]?.let { Log.d("MYCAt", it.toString()) }
            ...
            if (postData != null) {
                binding.txtHomeBigCat.text = postData.embedded?.wpTerm?.get(0)?.get(0)?.name
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        val bigView : HomeBigRowLayoutBinding

        return if (viewType == FIRST_VIEW_TYPE){
            bigView = HomeBigRowLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
