...
class FragmentI : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentIBinding.inflate(inflater, container, false)
        mAdapter = VerticalAdapter2(requireContext())
        mRecyclerView = binding.fragmentRecyclerI
        ...
        mainViewModel.postResponseI.observe(viewLifecycleOwner, {
            ...
            binding.refreshDataI.isRefreshing = false
            hideShimmerEffect()
        })

        binding.refreshDataI.setOnRefreshListener {
            showShimmerEffect()
            pageNo=1
            postFinish = 1
            mAdapter.clearList()
            mainViewModel.apiCall( this_category, TYPE_SINGLE, perPage, pageNo, FRAGMENT_NAME_I )
        }
